...
vector<string> reg_name = {"zero", "ra", "sp",  "gp",  "tp", "t0", "t1", "t2",
                           "fp",   "s1", "a0",  "a1",  "a2", "a3", "a4", "a5",
                           "a6",   "a7", "s2",  "s3",  "s4", "s5", "s6", "s7",
                           "s8",   "s9", "s10", "s11", "t3", "t4", "t5", "t6"};
/** for LLVM calling conventions */
const vector<InstGen::Reg> caller_save_regs = {
    InstGen::Reg(1),  InstGen::Reg(5),  InstGen::Reg(6),  InstGen::Reg(7),
...
const vector<InstGen::Reg> callee_save_regs = {
    InstGen::Reg(2),  InstGen::Reg(8),  InstGen::Reg(9),  InstGen::Reg(18),
...
const vector<InstGen::Reg> temp_regs = {
    InstGen::Reg(5),  InstGen::Reg(6),  InstGen::Reg(7), InstGen::Reg(28),
    InstGen::Reg(29), InstGen::Reg(30), InstGen::Reg(31)};

class RiscVBackEnd {
   public:
    /** Error codes. */
    int ERROR_ARG = 1, ERROR_DIV_ZERO = 2, ERROR_OOB = 3, ERROR_NONE = 4,
        ERROR_OOM = 5, ERROR_NYI = 6;
    ...
    int EXIT_ECALL = 10, EXIT2_ECALL = 17, PRINT_STRING_ECALL = 4,
    ...
    string emit_prototype(Class classInfo) {
        ...
        for (auto attr : *classInfo.get_attribute()) {
            if (attr->init_obj != nullptr) {
                if (auto glob = dynamic_cast<GlobalVariable *>(attr->init_obj);
                    glob) {
                    asm_code += fmt::format("  .word {}\n", glob->get_name());
                } else {
     ...
     */
    template <typename T>
    static string defineSym(string name, T value) {  // deprecated
        if (name.substr(0, 1) == "@") {
     ...
     * if non-null.
     */
    static string emit_ecall(string comment = "") {
     ...
     */
    static string emit_la(const InstGen::Reg &rd, InstGen::Addr label,
                          string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     */
    static string emit_li(const InstGen::Reg &rd, int imm,
                          string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("li {}, {}", rd.get_name(), imm),
                           comment);
    }
     ...
     */
    static string emit_lui(const InstGen::Reg &rd, int imm,
                           string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("lui {}, {}", rd.get_name(), imm),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_mv(const InstGen::Reg &rd, const InstGen::Reg &rs,
                          string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_neg(const InstGen::Reg &rd, const InstGen::Reg &rs,
                           string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     */
    static string emit_addi(const InstGen::Reg &rd, const InstGen::Reg &rs,
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_add(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Value &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("add {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_sub(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Value &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("sub {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_mul(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("mul {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_div(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("div {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     */
    static string emit_rem(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("rem {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * one-line comment (null if missing).
     */
    static string emit_xor(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("xor {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * one-line comment (null if missing).
     */
    static string emit_xori(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_and(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("and {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_andi(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_or(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                          const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("or {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_ori(const InstGen::Reg &rd, const InstGen::Reg &rs,
                           int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_sll(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("sll {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_slli(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_srl(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("srl {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_srli(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_sra(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("sra {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_srai(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_lw(const InstGen::Reg &rd, const InstGen::Reg &rs,
                          int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("lw {}, {}({})", rd.get_name(), imm, rs.get_name()),
            comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_sw(const InstGen::Reg &rs2, const InstGen::Reg &rs1,
                          int imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_lw(const InstGen::Reg &rs, InstGen::Addr label,
                          string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_sw(const InstGen::Reg &rs, InstGen::Addr label,
                          const InstGen::Reg &tmp, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("sw {}, {}, {}", rs.get_name(),
                                       label.get_name(), tmp.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_lb(const InstGen::Reg &rd, const InstGen::Reg &rs,
                          const int &imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("lb {}, {}, {}", rd.get_name(), imm, rs.get_name()),
            comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_lbu(const InstGen::Reg &rd, const InstGen::Reg &rs,
                           const int &imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_sb(const InstGen::Reg &rs2, const InstGen::Reg &rs1,
                          const int &imm, string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_beq(const InstGen::Reg &rs1, const InstGen::Reg &rs2,
                           InstGen::Addr label, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("beq {}, {}, {}", rs1.get_name(),
                                       rs2.get_name(), label.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bne(const InstGen::Reg &rs1, const InstGen::Reg &rs2,
                           InstGen::Addr label, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("bne {}, {}, {}", rs1.get_name(),
                                       rs2.get_name(), label.get_name()),
                           comment);
    }
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bge(const InstGen::Reg &rd, const InstGen::Reg &rs,
                           string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bgeu(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_blt(const InstGen::Reg &rd, const InstGen::Reg &rs,
                           string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bltu(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bnqz(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bnez(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bltz(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bgtz(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_blez(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_bgez(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_slt(const InstGen::Reg &rd, const InstGen::Reg &rs1,
                           const InstGen::Reg &rs2, string comment = "") {
        return fmt::format("  {:<40}#{:<42}\n",
                           fmt::format("slt {}, {}, {}", rd.get_name(),
                                       rs1.get_name(), rs2.get_name()),
                           comment);
    };
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_seqz(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
     ...
     * COMMENT is an optional one-line comment (null if missing).
     */
    static string emit_snez(const InstGen::Reg &rd, const InstGen::Reg &rs,
                            string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
    ...
    static string emit_vload(const InstGen::Reg &rd, const InstGen::Reg &rs,
                             string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("vload {}, {}", rd.get_name(), rs.get_name()), comment);
    };
    static string emit_vstore(const InstGen::Reg &rd, const InstGen::Reg &rs,
                              string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("vstore {}, {}", rd.get_name(), rs.get_name()),
            comment);
    };
    static string emit_vadd_vv(const InstGen::Reg &rd, const InstGen::Reg &rs,
                               string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("vadd.vv {}, {}", rd.get_name(), rs.get_name()),
            comment);
    };
    static string emit_vdiv_vv(const InstGen::Reg &rd, const InstGen::Reg &rs,
                               string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("vdiv.vv {}, {}", rd.get_name(), rs.get_name()),
            comment);
    };
    static string emit_vmul_vv(const InstGen::Reg &rd, const InstGen::Reg &rs,
                               string comment = "") {
        return fmt::format(
            "  {:<40}#{:<42}\n",
            fmt::format("vmul.vv {}, {}", rd.get_name(), rs.get_name()),
            comment);
    };

    static string emit_epilogue(const std::vector<InstGen::Reg> &reg_list,
                                string comment = "") {
        std::string asm_code;
        int count = 0;
        for (auto &i : reg_list) {
            asm_code += emit_sw(i, InstGen::Reg(0), 4 * count,
                                fmt::format(" Restore {} register", count));
            count++;
        }
        asm_code += indent + "ret\n";
        return asm_code;
