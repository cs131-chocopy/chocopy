...
struct LocationInfo {
    LocationInfo(string file, int line, const char *func)
        : file_(std::move(file)), line_(line), func_(func) {}
...
class LogWriter {
   public:
    LogWriter(const LocationInfo &location, LogLevel loglevel)
...
template <typename E>
constexpr auto get_underlying(E e) noexcept {
